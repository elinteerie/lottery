...
contract WalletDistributor {
    ...
    /// Track all owners who ever created a will
    ...
    gtUint64 encryptedOwnerBalance = MpcCore.onBoard(amountToShare[owner][willId].ciphertext);
    ...
    for (uint256 i = 0; i < wallets.length; i++) {
        ...
        // Track in MPC
        gtUint64 encryptedRecipientBalance = MpcCore.setPublic64(0);
        gtUint64 encryptedShare = MpcCore.setPublic64(uint64(share));

        (encryptedOwnerBalance, encryptedRecipientBalance, ) =
            MpcCore.transfer(encryptedOwnerBalance, encryptedRecipientBalance, encryptedShare);

    
    }

    // Update the owner’s encrypted balance
    amountToShare[owner][willId] = MpcCore.offBoardCombined(encryptedOwnerBalance, owner);

    willBalances[owner][willId] = 0;
    distributionTimestamp[owner][willId] = 0;
    inactivityPeriod[owner][willId] = 0;
}
